...
## Request Compression (IMPORTANT)

The frontend now compresses large design payloads using gzip. Your backend must accept gzip-compressed requests.
...
// Parse JSON bodies (including compressed)
app.use(express.json({ limit: '50mb' }))
app.use(express.raw({ type: 'application/gzip', limit: '50mb' }))

// Middleware to decompress gzip requests
  ...
  if (req.headers['content-encoding'] === 'gzip') {
    ...
    req.on('end', () => {
      ...
      try {
        ...
        res.status(400).json({ error: 'Failed to decompress gzip data' })
      }
    })
  } else {
    next()
...
### Alternative: Increase body parser limit

```javascript
// If not using compression, increase the limit
app.use(express.json({ limit: '50mb' }))
```

**Note:** Default Express body parser limit is typically 100KB. Designs with many components can exceed this.
...
### CRITICAL: Create Files Before index.ts

**WRONG - Exporting non-existent files:**
```javascript
// This fails if About.tsx doesn't exist!
export { default as About } from './About'  // Error: File not found
```

**RIGHT - Generate files first, then index.ts:**
```javascript
// Step 1: Generate all component files that are used
const usedTypes = new Set()
design.pages.forEach(page => {
  page.components.forEach(comp => usedTypes.add(comp.type))
})

// Step 2: Create each component file
for (const type of usedTypes) {
  const componentCode = await getComponentTemplate(type)
  await writeFile(`src/components/${type}.tsx`, componentCode)
}

// Step 3: Create index.ts ONLY for components that exist
const exports = Array.from(usedTypes).map(type => 
  `export { default as ${type} } from './${type}'`
).join('\n')
await writeFile('src/components/index.ts', exports)
```

### Step 4: Verify Files Exist Before Building

**CRITICAL - Always verify files were written:**
```javascript
const fs = require('fs').promises
const path = require('path')

async function verifyAndBuild(projectDir) {
  const componentsDir = path.join(projectDir, 'src/components')
  
  // Check each component file exists and has content
  const requiredFiles = ['Header.tsx', 'Home.tsx', 'Footer.tsx', 'About.tsx']
  for (const file of requiredFiles) {
    ...
    try {
      ...
      console.error(`✗ ${file} missing or invalid:`, e.message)
      throw e
    }
  }
...
### Common File Writing Issues

**Issue: Async file write not awaited**
```javascript
// WRONG - Fire and forget
writeFile('src/components/Header.tsx', code)  // Not awaited!
buildProject()  // May run before write completes

// RIGHT - Await all writes
await writeFile('src/components/Header.tsx', code)
await buildProject()
```

**Issue: Wrong directory path**
```javascript
// WRONG - Relative path from wrong cwd
process.chdir('/some/other/dir')
writeFile('src/components/Header.tsx', code)  // Writes to wrong place!

// RIGHT - Absolute paths
const projectDir = `/tmp/projects/${projectName}`
await writeFile(path.join(projectDir, 'src/components/Header.tsx'), code)
```
...
If using dynamic component imports, add:
```typescript
