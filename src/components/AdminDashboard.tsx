...
function StatCard({ label, value, icon: Icon, color, bg, trend, delay = 0 }: StatCardProps) {
  return (
    <div
...
function SkeletonCard() {
  return (
    <div className="rounded-2xl p-5 bg-white border border-slate-100 animate-pulse">
      <div className="flex items-start justify-between">
...
export default function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen p-6 bg-slate-50" style={{ background: 'hsl(220 20% 98%)' }}>
      {/* Page header */}
      <div className="flex items-center justify-between mb-8">
        <div className="animate-slide-in-left">
          <h1 className="text-2xl font-bold text-slate-900">Хянах самбар</h1>
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium text-slate-600 bg-white border border-slate-200 hover:border-indigo-300 hover:text-indigo-600 transition-all duration-200 shadow-sm disabled:opacity-50"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
        ...
        <div className="lg:col-span-2 rounded-2xl bg-white border border-slate-100 p-6 shadow-sm animate-fade-in-up" style={{ animationDelay: '200ms' }}>
          <div className="flex items-center justify-between mb-5">
            <h2 className="text-sm font-semibold text-slate-900">Төслийн төлөв</h2>
            <span className="text-xs text-slate-400">Нийт {projects.length}</span>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-8 text-center">
              <Server className="w-10 h-10 text-slate-200 mb-3" />
              <p className="text-sm text-slate-400">Төсөл алга</p>
            </div>
          ) : (
                ...
                <div key={item.label}>
                  <div className="flex items-center justify-between text-xs mb-1.5">
                    ...
                    <span className="text-slate-500 font-medium">
                      {item.count} <span className="text-slate-300">/ {projects.length}</span>
                    </span>
                  </div>
        ...
        {/* User breakdown */}
        <div className="rounded-2xl bg-white border border-slate-100 p-6 shadow-sm animate-fade-in-up" style={{ animationDelay: '260ms' }}>
          <div className="flex items-center justify-between mb-5">
            <h2 className="text-sm font-semibold text-slate-900">Хэрэглэгчийн эрх</h2>
          </div>
          {isLoading ? (
            <div className="space-y-3">{[...Array(3)].map((_, i) => <div key={i} className="h-10 bg-slate-50 animate-pulse rounded-xl" />)}</div>
          ) : users.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-8 text-center">
              <Users className="w-10 h-10 text-slate-200 mb-3" />
              <p className="text-sm text-slate-400">Хэрэглэгч алга</p>
            </div>
          ) : (
            <div className="space-y-2">
              {(['superadmin', 'client-admin', 'editor'] as const).map(role => {
                ...
                const styles = {
                  superadmin:    { label: 'Супер Админ',  color: 'bg-violet-50 text-violet-700 border-violet-100' },
                  'client-admin':{ label: 'Харилцагч Админ', color: 'bg-indigo-50 text-indigo-700 border-indigo-100' },
                  editor:        { label: 'Засварлагч',       color: 'bg-slate-50 text-slate-700 border-slate-100'    },
                }
                const s = styles[role]
                return (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {/* Recent Projects */}
        <div className="rounded-2xl bg-white border border-slate-100 shadow-sm animate-fade-in-up" style={{ animationDelay: '320ms' }}>
          ...
          <div className="divide-y divide-slate-50">
            {isLoading ? (
              ...
              recentProjects.map(p => {
                ...
                return (
                  <div key={p.name} className="px-6 py-3 flex items-center justify-between hover:bg-slate-50/50 transition-colors">
                    <div>
                      <p className="text-sm font-medium text-slate-800">{p.name}</p>
                      <div className="flex items-center gap-2 mt-0.5">
                        <Globe className="w-3 h-3 text-slate-400" />
                        <span className="text-xs text-slate-400">Порт {p.port}</span>
                        <Database className="w-3 h-3 text-slate-400" />
                        <span className="text-xs text-slate-400">{p.dbStatus}</span>
                      </div>
                    </div>
        ...
        {/* Recent Users */}
        <div className="rounded-2xl bg-white border border-slate-100 shadow-sm animate-fade-in-up" style={{ animationDelay: '380ms' }}>
          <div className="px-6 py-4 border-b border-slate-50 flex items-center justify-between">
            <h2 className="text-sm font-semibold text-slate-900 flex items-center gap-2">
            ...
            {isLoading ? (
              ...
              recentUsers.map(u => {
                ...
                const roleMap: Record<string, { label: string; color: string }> = {
                  superadmin:    { label: 'Супер Админ',  color: 'bg-violet-50 text-violet-700' },
                  'client-admin':{ label: 'Харилцагч Админ', color: 'bg-indigo-50 text-indigo-700' },
                  editor:        { label: 'Засварлагч',       color: 'bg-slate-50 text-slate-600'   },
                }
                const r = roleMap[u.role] || { label: u.role, color: 'bg-slate-50 text-slate-600' }
                return (
                  <div key={u.email} className="px-6 py-3 flex items-center gap-3 hover:bg-slate-50/50 transition-colors">
                    <div
